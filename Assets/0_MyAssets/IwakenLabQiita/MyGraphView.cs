...
namespace MU5Editor.NodeEditor
{
    ...
    public class MyGraphView : GraphView
    {
        ...
        public Dictionary<string, MU5Node> Node_dict
        {
            get
            {
                Dictionary<string, MU5Node> node_dict = new Dictionary<string, MU5Node>();
                foreach (var element in graphElements)
                {
                    if (element is MU5Node)
                    {
                        MU5Node node = (MU5Node)element;
                        node_dict.Add(node.uid, node);
                    }
                }
                return node_dict;
            }
